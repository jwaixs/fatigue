...
BOOST_AUTO_TEST_CASE(memory_problem) {
  ...
  BOOST_CHECK_MESSAGE(
      mp.correctNumber(problem) == problem.size(),
  ...
  for (unsigned long i = 0; i < problem.size(); i++) {
    ...
    BOOST_CHECK_MESSAGE(mp.correctNumber(guess) == i,
                        "First " + std::to_string(i) +
                            " correct characters should give " +
...
BOOST_AUTO_TEST_CASE(statistic) {
  ...
  BOOST_CHECK_MESSAGE(
      ps.getNumberOfTries() == number_of_tries,
      "getNumberOfTries should be equal to " + std::to_string(number_of_tries));
  BOOST_CHECK_MESSAGE(
      ps.getTimePerTry().size() == tries.size(),
      "getTimePerTry size should be equal to " + std::to_string(tries.size()));
  ...
  std::vector<std::string> dates{"01-01-01", "02-02-02", "03-03-03", "04-04-04",
                                 "05-05-05"};
  ...
  BOOST_CHECK_MESSAGE(
      psd.getNumberOfTries() == number_of_tries,
      "getNumberOfTries should be equal to " + std::to_string(number_of_tries));
  BOOST_CHECK_MESSAGE(
      psd.getTimePerTry().size() == tries.size(),
      "getTimePerTry size should be equal to " + std::to_string(tries.size()));
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(psd_dates.begin(), psd_dates.end(),
...
BOOST_AUTO_TEST_CASE(time_tools) {
  ...
  BOOST_CHECK_MESSAGE(getWeek(current_ptime) == 8, "getWeek should be 8.");
  BOOST_CHECK_MESSAGE(getYear(current_ptime) == 2018,
                      "getYear should be 2018.");

  auto const current_time = current_ptime.time_of_day();
  BOOST_CHECK_MESSAGE(current_time.hours() == 11, "Current hour should be 11.");
...
BOOST_AUTO_TEST_CASE(cumulative_function) {
  ...
  std::vector<float> const result{0.0, 0.1, 0.3, 0.3, 0.4,
                                  0.5, 0.8, 0.9, 1.0, 1.0};
  for (unsigned int i = 0; i <= 9; i++) {
  ...
  auto const correct_step_positions =
      std::vector<float>{1.0, 2.0, 4.0, 5.0, 6.0, 7.0, 8.0};
  for (unsigned int i = 0; i < cf2_step_positions.size(); i++) {
    BOOST_CHECK_MESSAGE(
        std::abs(cf2_step_positions.at(i) - correct_step_positions.at(i)) <
            tolerance,
